	...
	case Token.LOOP:
	{
		...
		switch(currentToken){
			...
			case Token.DO:
			{
				acceptIt();
				Command cAST = parseCommand();
				if(currentToken.equals(Token.WHILE)){
					accepIt();
					Expression eAST = parseExpression();
					accept(Token.REPEAT);
					finish(commandPos);
					commandAST = new DoWhileCommand(cAST, eAST, commandPos);
				}else if (currentToken.equals(Token.UNTIL)){
					accepIt();
					Expression eAST = parseExpression();
					accept(Token.REPEAT);
					finish(commandPos);
    ...
    case Token.VAR:
      {
        acceptIt();
        Identifier iAST = parseIdentifier();
		if(currenteToken.equals(Token.COLON)){
			acceptIt();
			TypeDenoter tAST = parseTypeDenoter();
			finish(declarationPos);
			declarationAST = new VarDeclaration(iAST, tAST, declarationPos);
		}
		else if(currenteToken.equals(Token.INIT)){
			acceptIt();
			Expression eAST = parseExpression();
			finish(declarationPos);
			declarationAST = new InitializedVarDeclaration(iAST,eAST,declarationPos);
		}else{
			syntacticError("\"%\" cannot start a command, expected colon or init",currentToken.spelling);
		}
      }
      break;
